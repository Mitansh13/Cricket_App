...
const dummyStudents: Student[] = [
  {
    ...
    role: "Batsman",
    careerInfo: "Aggressive right-handed batsman with excellent technique and leadership qualities.",
    isMyStudent: true,
    joinDate: "2024-01-15",
  ...
  },
  {
    ...
    role: "Opener",
    careerInfo: "Elegant opener known for his timing and ability to score big centuries.",
    isMyStudent: true,
    joinDate: "2024-02-01",
  ...
  },
  {
    ...
    role: "Fast Bowler",
    careerInfo: "Right-arm fast bowler with unique action and excellent death bowling skills.",
    isMyStudent: false,
    joinDate: "2024-03-10",
  ...
  },
  {
    ...
    role: "All-rounder",
    careerInfo: "Dynamic all-rounder who can change the game with both bat and ball.",
    isMyStudent: false,
    joinDate: "2024-01-20",
  ...
  },
  {
    ...
    careerInfo: "Versatile wicket-keeper batsman who can adapt to any batting position.",
    isMyStudent: true,
    joinDate: "2024-02-15",
  ...
  const handleAcceptRequest = async (id: string) => {
    try {
      ...
      if (request) {
        ...
        Alert.alert("Success", `${request.name} has been accepted as your student!`);
      }
    } catch (error) {
    ...
    >
      ...
      <View style={styles.header}>
        <View style={styles.profile}>
          <Image
            source={profileUrl ? { uri: profileUrl } : require("../../assets/images/boy.png")}
            style={styles.profileImage}
          />
          <View style={styles.greetingContainer}>
            <Text style={styles.greetingText}>Hello, {coachName || "Coach"}</Text>
      ...
      <View style={styles.statsGrid}>
        <TouchableOpacity style={[styles.statBox, styles.blueBox]} onPress={handleMyStudents}>
          <Feather name="users" size={24} color="#fff" />
          <Text style={styles.statLabel}>My Students</Text>
          <Text style={styles.statValue}>{stats.myStudents}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.statBox, styles.purpleBox]} onPress={handleTotalStudents}>
          <Feather name="globe" size={24} color="#fff" />
          <Text style={styles.statLabel}>Total Students</Text>
          <Text style={styles.statValue}>{stats.totalStudents}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.statBox, styles.greenBox]} onPress={handleCalendar}>
          <Feather name="calendar" size={24} color="#fff" />
          <Text style={styles.statLabel}>Events</Text>
          <Text style={styles.statValue}>{stats.events}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.statBox, styles.orangeBox]} onPress={handleVideos}>
        ...
        {joinRequests.length === 0 ? (
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {joinRequests.map((req) => (
              ...
              >
                <Image source={{ uri: req.profileUrl }} style={styles.requestImage} />
                <View style={styles.requestInfo}>
                  <Text style={styles.requestName}>{req.name}</Text>
                  <Text style={styles.requestRole}>{req.role}</Text>
                </View>
                <View style={styles.requestActions}>
                  <TouchableOpacity style={styles.quickAcceptButton} onPress={() => handleAcceptRequest(req.id)}>
                    <Feather name="check" size={16} color="#fff" />
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.quickRejectButton} onPress={() => handleRejectRequest(req.id)}>
      ...
      <View style={styles.eventsContainer}>
        <View style={styles.sectionHeader}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <TouchableOpacity onPress={handleCalendar} style={{ marginRight: 15 }}>
      ...
      <View style={styles.videosContainer}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {videos.slice(0, 5).map((video) => (
            <TouchableOpacity key={video.id} style={styles.videoCard} onPress={handleVideos}>
      ...
      <Modal visible={!!selectedRequest} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {selectedRequest && (
              <>
                <Image source={{ uri: selectedRequest.profileUrl }} style={styles.modalImage} />
                  ...
                  <View style={styles.performanceContainer}>
                    ...
                    <View style={styles.performanceMetrics}>
                      <View style={styles.metricItem}>
                        ...
                        <Text style={styles.metricValue}>{selectedRequest.performance.batting}%</Text>
                      </View>
                      <View style={styles.metricItem}>
                        <Text style={styles.metricLabel}>Bowling</Text>
                        <Text style={styles.metricValue}>{selectedRequest.performance.bowling}%</Text>
                      </View>
                      <View style={styles.metricItem}>
                        <Text style={styles.metricLabel}>Fielding</Text>
                        <Text style={styles.metricValue}>{selectedRequest.performance.fielding}%</Text>
                      </View>
                    </View>
                  </View>
                )}
                <View style={styles.modalActions}>
                  <TouchableOpacity style={[styles.actionButton, styles.acceptButton]} onPress={() => handleAcceptRequest(selectedRequest.id)}>
                    <Feather name="check" size={20} color="#fff" />
                    <Text style={styles.actionButtonText}>Accept</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.actionButton, styles.rejectButton]} onPress={() => handleRejectRequest(selectedRequest.id)}>
    ...
    <View style={styles.modalContent}>
      ...
      >
        ...
        <View style={styles.formGroup}>
          ...
          <View style={styles.typeSelector}>
            {(['session', 'match', 'workshop', 'meeting'] as const).map(type => (
              <TouchableOpacity
                key={type}
                style={[
                  styles.typeButton,
      ...
      <View style={styles.modalActions}>
        ...
        >
          ...
          <Text style={styles.actionButtonText}>{editingEvent ? 'Update' : 'Save'}</Text>
        </TouchableOpacity>
      </View>
    </View>
  </View>
