...
const explainerIcons = ["information", "lightbulb-on", "comment-question"] as const;
...
const CoachAnnotationResultScreen = () => {
  ...
  const [exercises, setExercises] = useState<{ name: string; icon: ExerciseIcon }[]>([]);
  const [explainers, setExplainers] = useState<{ name: string; icon: ExplainerIcon }[]>([]);
  ...
  const shareResult = async () => {
    try {
      const selectedDrills = suggestedDrills.filter((d) => selectedDrillIds.includes(d.id));
      await Share.share({
  ...
  const [newExerciseIcon, setNewExerciseIcon] = useState<ExerciseIcon>("barbell");
  const [newExplainerName, setNewExplainerName] = useState("");
  const [newExplainerIcon, setNewExplainerIcon] = useState<ExplainerIcon>("information");
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.container} contentContainerStyle={{ padding: 20 }}>
        ...
        <View style={styles.header}>
          <Image
            source={
              videoThumbnail ? { uri: videoThumbnail } : require("../../assets/images/cricket.jpg")
            }
            style={styles.thumbnail}
          />
          <Text style={styles.sessionTitle}>{sessionTitle || "Session Title"}</Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          {suggestedDrills.map((drill) => (
            <TouchableOpacity
              ...
              style={[
                styles.drillItem,
                selectedDrillIds.includes(drill.id) && { backgroundColor: "#E0E7FF" },
              ]}
              onPress={() => {
            ...
            >
              <MaterialCommunityIcons name={drill.icon} size={24} color="#1D4ED8" />
        ...
        <View style={styles.section}>
          ...
          {exercises.map((ex, idx) => (
            <View key={idx} style={[styles.drillItem, { justifyContent: "space-between" }]}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <MaterialCommunityIcons name={ex.icon} size={24} color="#1D4ED8" />
        ...
        {/* Save Button */}
        <TouchableOpacity style={[styles.shareButton, { backgroundColor: "#1D4ED8", marginTop: 10 }]} onPress={saveFeedback}>
          ...
          <View style={styles.modalContainer}>
            ...
            <View style={styles.iconRow}>
              {exerciseIcons.map((icon) => (
                ...
                >
                  <MaterialCommunityIcons name={icon} size={30} color="#1D4ED8" />
                </TouchableOpacity>
              ))}
            </View>
